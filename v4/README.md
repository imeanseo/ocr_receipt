...
### 젤 잘 나온 epoch로 predict → CSV 한 번에 (권장)
```bash
# v3 디렉터리에서 실행
# 체크포인트 지정 (WandB 등에서 best epoch 확인 후)
python scripts/run_submission.py outputs/ocr_training/checkpoints/epoch=5-step=546.ckpt

# 체크포인트 미지정 시 checkpoints/ 안에서 가장 최신 .ckpt 사용
python scripts/run_submission.py
```
- 예측 결과 JSON은 `outputs/ocr_training/submissions/{timestamp}.json`에 저장됨
- 제출용 CSV는 `outputs/ocr_training/submissions/submission.csv`로 자동 생성됨
